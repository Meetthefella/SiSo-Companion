...
import { visibleKitsForGroup } from './kitBoard.js';
import type { AuditResult, AuditSession, BulkCount, InventoryAsset, KitCatalogEntry, KitCheck, KitComparisonStatus, ManageBookingRow, OcrCandidate, QueueStatus, ReconcileOutcome } from './types';
...
function kitCounts() {
  const all = uniqueKits(), checked = new Map(kitChecks.map(k => [k.kit_barcode, k]));
  const c: { all: number; not_checked: number; reconciled: number; collected: number; second_pass: number; further_action: number } = { all: all.length, not_checked: 0, reconciled: 0, collected: 0, second_pass: 0, further_action: 0 };
  for (const kit of all) { const check = checked.get(kit.barcode); if (!check) c.not_checked++; else if (check.comparison_status === 'reconciled') c.reconciled++; else if (check.comparison_status === 'collected') c.collected++; else if (check.comparison_status === 'second_pass') c.second_pass++; else c.further_action++; }
  return c;
}
...
function boardSectionsHtml() { const kits = uniqueKits(), groups = [...new Set(kits.map(k => k.group))], checked = new Map(kitChecks.map(k => [k.kit_barcode, k])); return groups.map(group => { const groupKits = visibleKitsForGroup(kits, group, kitStatusFilter, checked); if (!groupKits.length) return ''; const selectedCount = groupKits.filter(k => selectedPresentKits.has(k.barcode)).length; return `<section class="kit-group-section"><div class="kit-group-header"><div><h3>${esc(group)}</h3><span>${groupKits.length} shown${selectedCount ? ` · ${selectedCount} selected present` : ''}</span></div><button data-reconcile-group="${esc(group)}" class="small-action" ${!bookingRows.length || isReadOnly() ? 'disabled' : ''}>Reconcile ${esc(group)}</button></div><div class="kit-wall">${groupKits.map(k => { const prior = checked.get(k.barcode), selected = selectedPresentKits.has(k.barcode), status = kitStatus(prior), cls = selected ? 'selected' : `status-${status}`; return `<button class="kit-tile ${cls}" data-kit="${k.barcode}" aria-label="${esc(k.code)} ${esc(kitStatusLabel(status))}" ${isReadOnly() && !prior ? 'disabled' : ''}><strong>${esc(k.number)}</strong><span>${esc(kitStatusLabel(status))}</span></button>` }).join('')}</div></section>`; }).join(''); }
...
function dashboardHtml(c: Record<QueueStatus, number>) {
...
async function recalculateKitChecks() { if (!currentSession || isReadOnly() || !kitChecks.length) return; const records = kitChecks.map(check => { const active = bookingFor(check.kit_barcode).find(b => isOutState(b.state)); const present = check.physical_state === 'present'; const comparison_status: KitComparisonStatus = present && active ? 'siso_update' : !present && !active ? 'missing' : !present && active ? 'collected' : 'reconciled'; const detail = present && active ? `Physically present but SiSo shows ${active.state}.` : !present && !active ? 'Not physically present and not shown as out in Manage Bookings.' : !present && active ? `Physically absent and correctly shown as ${active.state}.` : 'Physically present and not shown as out.'; return { ...check, booking_state: active?.state ?? null, comparison_status, detail, updated_at: new Date().toISOString() }; }); const { error } = await supabase.from('kit_checks').upsert(records, { onConflict: 'audit_session_id,kit_barcode' }); if (error) throw error; }
async function reconcileKitGroup(group = selectedKitGroup) { if (!currentSession || !group || isReadOnly()) return; const groupKits = visibleKitsForGroup(uniqueKits(), group, kitStatusFilter, new Map(kitChecks.map(k => [k.kit_barcode, k]))); const now = new Date().toISOString(); const records = groupKits.map(k => { const present = selectedPresentKits.has(k.barcode), bookings = bookingFor(k.barcode), active = bookings.find(b => isOutState(b.state)); let comparison_status: KitComparisonStatus = 'reconciled', detail: string | null = null; if (present && active) { comparison_status = 'siso_update'; detail = `Physically present but SiSo shows ${active.state}${active.booked_by ? ` to ${active.booked_by}` : ''}.`; } else if (!present && !active) { comparison_status = 'missing'; detail = 'Not physically present and not shown as out in Manage Bookings.'; } else if (!present && active) { comparison_status = 'collected'; detail = `Physically absent and correctly shown as ${active.state}${active.booked_by ? ` to ${active.booked_by}` : ''}.`; } else { detail = 'Physically present and not shown as out.'; } return { audit_session_id: currentSession!.id, kit_barcode: k.barcode, kit_code: k.code, kit_group: k.group, physical_state: present ? 'present' : 'absent', booking_state: active?.state ?? null, comparison_status, detail, checked_by: technician, checked_at: now, updated_at: now }; }); const { error } = await supabase.from('kit_checks').upsert(records, { onConflict: 'audit_session_id,kit_barcode' }); if (error) alert(error.message); else { for (const k of groupKits) selectedPresentKits.delete(k.barcode); await loadSessionData(); render(); } }
...
function bindEvents() {
  ...
  document.querySelector('#change-session')?.addEventListener('click', () => { storeBoardState(); currentSession = null; render(); });
  document.querySelectorAll<HTMLButtonElement>('[data-select-session]').forEach(b => b.addEventListener('click', async () => { storeBoardState(); currentSession = sessions.find(s => s.id === b.dataset.selectSession) ?? null; selectedPresentKits.clear(); restoreBoardState(); await loadSessionData(); render(); await subscribe(); }));
  ...
  document.querySelector('#create-session')?.addEventListener('click', async () => { const name = (document.querySelector<HTMLInputElement>('#new-session-name')?.value ?? '').trim(); if (!name || !authUserId) return; const { data, error } = await supabase.from('audit_sessions').insert({ name, created_by: technician, lead_user_id: authUserId, lead_name: technician }).select().single(); if (error) alert(error.message); else { currentSession = data; await refresh(); } });
  document.querySelector('#claim-lead')?.addEventListener('click', async () => { if (!currentSession) return; const { error } = await supabase.rpc('claim_reconciliation_lead', { p_audit_session_id: currentSession.id, p_lead_name: technician }); if (error) alert(error.message); else await refresh(); });
  document.querySelector('#reset-progress')?.addEventListener('click', async () => { if (!currentSession || !confirm(`Reset all technician progress for “${currentSession.name}”? Stock and Manage Bookings will be kept.`)) return; const { error } = await supabase.rpc('reset_reconciliation_progress', { p_audit_session_id: currentSession.id }); if (error) alert(error.message); else await refresh(); });
  document.querySelector('#archive-session')?.addEventListener('click', async () => { if (!currentSession || !confirm(`Archive “${currentSession.name}”? It will become read-only.`)) return; const { error } = await supabase.rpc('archive_reconciliation', { p_audit_session_id: currentSession.id }); if (error) alert(error.message); else await refresh(); });
  document.querySelector('#delete-session')?.addEventListener('click', async () => { if (!currentSession || prompt(`Type the reconciliation name to delete it permanently:\n${currentSession.name}`) !== currentSession.name) return; const { error } = await supabase.rpc('delete_reconciliation', { p_audit_session_id: currentSession.id }); if (error) alert(error.message); else { currentSession = null; await refresh(); } });
  ...
  try {
    ...
    console.error('Manage Bookings refresh failed:', e);
      ...
      document.querySelector('#reopen-kit')?.addEventListener('click', async () => { const id = (document.querySelector<HTMLButtonElement>('#reopen-kit')?.dataset.kitCheckId); if (!id) return; const { error } = await supabase.from('kit_checks').delete().eq('id', id); if (error) alert(error.message); else { selectedKitBarcode = null; await loadSessionData(); render(); } });
...
async function refreshFromSessionSignal() { await loadSessions(); await loadSessionData(); restoreBoardState(); render(); }
async function subscribe() { if (realtimeChannel) await supabase.removeChannel(realtimeChannel); realtimeStatus = 'disconnected'; realtimeDetail = ''; if (!currentSession) return; const sessionId = currentSession.id; realtimeStatus = 'connecting'; realtimeChannel = supabase.channel(`audit:${sessionId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'audit_results', filter: `audit_session_id=eq.${sessionId}` }, async () => { await loadSessionData(); render(); }).on('postgres_changes', { event: '*', schema: 'public', table: 'bulk_counts', filter: `audit_session_id=eq.${sessionId}` }, async () => { await loadSessionData(); render(); }).on('postgres_changes', { event: '*', schema: 'public', table: 'kit_checks', filter: `audit_session_id=eq.${sessionId}` }, async () => { await loadSessionData(); render(); }).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'audit_sessions', filter: `id=eq.${sessionId}` }, async (payload: { new: Partial<AuditSession> }) => { const next = payload.new; if (next.booking_revision !== currentSession?.booking_revision || next.status !== currentSession?.status || next.lead_user_id !== currentSession?.lead_user_id) await refreshFromSessionSignal(); }).subscribe((status: string) => { realtimeDetail = status; if (status === 'SUBSCRIBED') realtimeStatus = 'subscribed'; else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') { realtimeStatus = 'error'; lastDebugError = `Realtime: ${status}`; } else if (status === 'CLOSED') realtimeStatus = 'disconnected'; else realtimeStatus = 'connecting'; if (activeView === 'debug') render(); }); }
    async function refresh() { await loadSessions(); await loadSessionData(); restoreBoardState(); render(); await subscribe(); }
