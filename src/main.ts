...
function render() { const c = counts(); app.innerHTML = `<div class="shell"><header><div><h1 id="debug-trigger">SiSo Companion</h1><p>${esc(currentSession?.name ?? 'No reconciliation selected')}</p></div>${currentSession ? `<button id="change-session" class="header-button">Change</button>` : ''}</header><main>${!technician ? techSetup() : viewHtml(c)}</main>${technician ? navHtml() : ''}${pendingCandidate ? reasonModal() : ''}${selectedKitBarcode ? kitDetailModal() : ''}</div>`; bindEvents(); }
...
function boardSectionsHtml() { const kits = uniqueKits(), groups = [...new Set(kits.map(k => k.group))], checked = new Map(kitChecks.map(k => [k.kit_barcode, k])); return groups.map(group => { const groupKits = kits.filter(k => k.group === group).filter(k => { const status = kitStatus(checked.get(k.barcode)); if (kitStatusFilter === 'all') return true; if (kitStatusFilter === 'further_action') return status === 'missing' || status === 'siso_update'; return status === kitStatusFilter; }); if (!groupKits.length) return ''; const selectedCount = groupKits.filter(k => selectedPresentKits.has(k.barcode)).length; return `<section class="kit-group-section"><div class="kit-group-header"><div><h3>${esc(group)}</h3><span>${groupKits.length} shown${selectedCount ? ` · ${selectedCount} selected present` : ''}</span></div><button data-reconcile-group="${esc(group)}" class="small-action" ${!bookingRows.length ? 'disabled' : ''}>Reconcile ${esc(group)}</button></div><div class="kit-wall">${groupKits.map(k => { const prior = checked.get(k.barcode), selected = selectedPresentKits.has(k.barcode), status = kitStatus(prior), cls = selected ? 'selected' : `status-${status}`; return `<button class="kit-tile ${cls}" data-kit="${k.barcode}" aria-label="${esc(k.code)} ${esc(kitStatusLabel(status))}"><strong>${esc(k.number)}</strong><span>${esc(kitStatusLabel(status))}</span></button>` }).join('')}</div></section>`; }).join(''); }
...
async function reconcileKitGroup(group = selectedKitGroup) { if (!currentSession || !group) return; const groupKits = uniqueKits().filter(k => k.group === group); const now = new Date().toISOString(); const records = groupKits.map(k => { const present = selectedPresentKits.has(k.barcode), bookings = bookingFor(k.barcode), active = bookings.find(b => isOutState(b.state)); let comparison_status: KitComparisonStatus = 'reconciled', detail: string | null = null; if (present && active) { comparison_status = 'siso_update'; detail = `Physically present but SiSo shows ${active.state}${active.booked_by ? ` to ${active.booked_by}` : ''}.`; } else if (!present && !active) { comparison_status = 'missing'; detail = 'Not physically present and not shown as out in Manage Bookings.'; } else if (!present && active) { comparison_status = 'collected'; detail = `Physically absent and correctly shown as ${active.state}${active.booked_by ? ` to ${active.booked_by}` : ''}.`; } else { detail = 'Physically present and not shown as out.'; } return { audit_session_id: currentSession!.id, kit_barcode: k.barcode, kit_code: k.code, kit_group: k.group, physical_state: present ? 'present' : 'absent', booking_state: active?.state ?? null, comparison_status, detail, checked_by: technician, checked_at: now, updated_at: now }; }); const { error } = await supabase.from('kit_checks').upsert(records, { onConflict: 'audit_session_id,kit_barcode' }); if (error) alert(error.message); else { for (const k of groupKits) selectedPresentKits.delete(k.barcode); await loadSessionData(); render(); } }
...
function bindEvents() {
  ...
  if (!file || !currentSession || !status) return;
