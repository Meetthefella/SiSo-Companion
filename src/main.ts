...
async function loadSessionData() {
  if (!currentSession) { assets = []; results = []; bulkCounts = []; bookingRows = []; kitCatalog = []; kitChecks = []; ignoredKitGroups = []; return; } const [a, r, b, m, c, k, i] = await Promise.all([
    ...
    supabase.from('ignored_kit_groups').select('*').eq('audit_session_id', currentSession.id)
  ]);
...
function boardSectionsHtml() { const items = reconciliationState(), groups = [...new Set(items.map(item => item.group))], checked = new Map(kitChecks.map(k => [k.kit_barcode, k])); return groups.map(group => { const allGroupItems = items.filter(item => item.group === group), ignored = allGroupItems[0]?.ignored ?? false, groupItems = ignored ? allGroupItems : visibleReconciliationItems(allGroupItems, group, kitStatusFilter); if (!groupItems.length) return ''; const selectedCount = groupItems.filter(item => selectedPresentKits.has(item.barcode)).length; return `<section class="kit-group-section ${ignored ? 'ignored-group' : ''}"><div class="kit-group-header"><div><h3>${esc(group)}</h3><span>${ignored ? `${allGroupItems.length} items ignored from totals and CSV exports` : `${groupItems.length} shown${selectedCount ? ` · ${selectedCount} selected present` : ''}`}</span></div>${isLead() && !isReadOnly() ? `<button data-toggle-ignore-group="${esc(group)}" data-ignore="${ignored ? 'false' : 'true'}" class="small-action ${ignored ? 'secondary' : 'warn'}">${ignored ? 'Include group' : 'Ignore group'}</button>` : ''}${!ignored ? `<button data-reconcile-group="${esc(group)}" class="small-action" ${!bookingRows.length || isReadOnly() ? 'disabled' : ''}>Reconcile ${esc(group)}</button>` : ''}</div>${ignored ? '' : `<div class="kit-wall">${groupItems.map(item => { const prior = checked.get(item.barcode), selected = selectedPresentKits.has(item.barcode), cls = selected ? 'selected' : `status-${item.status}`; return `<button class="kit-tile ${cls}" data-kit="${item.barcode}" aria-label="${esc(item.code)} ${esc(reconciliationStatusLabel(item.status))}" ${isReadOnly() && !prior ? 'disabled' : ''}><strong>${esc(item.number)}</strong><span>${esc(reconciliationStatusLabel(item.status))}</span></button>` }).join('')}</div>`}</section>`; }).join(''); }
...
async function reconcileKitGroup(group = selectedKitGroup) { if (!currentSession || !group || isReadOnly()) return; const groupItems = visibleReconciliationItems(reconciliationState(), group, kitStatusFilter); const now = new Date().toISOString(); const records = groupItems.map(item => { const present = selectedPresentKits.has(item.barcode), bookings = bookingFor(item.barcode), active = bookings.find(b => isOutState(b.state)); let comparison_status: KitComparisonStatus = 'reconciled', detail: string | null = null; if (present && active) { comparison_status = 'siso_update'; detail = `Physically present but SiSo shows ${active.state}${active.booked_by ? ` to ${active.booked_by}` : ''}.`; } else if (!present && !active) { comparison_status = 'missing'; detail = 'Not physically present and not shown as out in Manage Bookings.'; } else if (!present && active) { comparison_status = 'collected'; detail = `Physically absent and correctly shown as ${active.state}${active.booked_by ? ` to ${active.booked_by}` : ''}.`; } else { detail = 'Physically present and not shown as out.'; } return { audit_session_id: currentSession!.id, kit_barcode: item.barcode, kit_code: item.code, kit_group: item.group, physical_state: present ? 'present' : 'absent', booking_state: active?.state ?? null, comparison_status, detail, checked_by: technician, checked_at: now, updated_at: now }; }); const { error } = await supabase.from('kit_checks').upsert(records, { onConflict: 'audit_session_id,kit_barcode' }); if (error) alert(error.message); else { for (const item of groupItems) selectedPresentKits.delete(item.barcode); await loadSessionData(); render(); } }
